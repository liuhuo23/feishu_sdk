...
impl FeishuSheet {
    ...
    pub async fn write_line(&self, range: &str, value: Vec<Value>) {
        let values: Vec<Vec<Value>> = vec![value];
        self.write(range, values).await;
    }
    pub async fn write(&self, range: &str, value: Vec<Vec<Value>>) {
        let json_str = format!(
            r#"{{
            "valueRange": {{
                "range": "{range}",
        ...
        debug!("json: {}", json_str);
        let value = self.send_json(&json_str).await;
        println!("{}", value);
    }
    pub async fn read(&self, range: &str) -> Value {
        let url = format!(
    ...
    async fn send_json(&self, json_str: &str) -> Value {
    ...
    pub async fn add_sheet(&self, sheet_title: &str) -> FeishuSheet {
        let url = format!(
